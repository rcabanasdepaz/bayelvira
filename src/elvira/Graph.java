...
/**
 * Saves the list of nodes in the text output stream given as parameter in
 * a matlab format.
 * @param p the <code>PrintWriter</code> where the list will be saved.
 */

public void saveNodeListMatlab(PrintWriter p) {

  p.println("%%% Information about the nodes %%%");  
  p.print("NODES={}; KIND={};\n");

  Node n = null;
  for (int i=0 ; i<getNodeList().size() ; i++) {
    n = ((Node) getNodeList().elementAt(i));
    p.println("NODES={NODES{:} '"+n.getName()+"'}; KIND={KIND{:} '"+n.getKind()+"'};");
...
/**
 * Saves the links of the graph in the variable given as argument using the matlab format.
 * @param p text output stream used to saved the links of the graph.
 */


public void saveLinkListMatlab(PrintWriter p) {

  p.println("%%% Information about the links %%%");  
  p.print("LINKS={};\n");

  Link l = null;
  for (int i=0 ; i<getLinkList().size() ; i++) {
    l = ((Link) getLinkList().elementAt(i));
    p.println("LINKS={LINKS{:} {'"+l.getTail().getName()+"', '"+l.getHead().getName()+"', "+l.getDirected()+"}};");
  }
  p.println("%%%%%%%%%"); 
...
/**
 * Saves the graph in the variable given as argument using the matlab format.
 * @param p text output stream used to saved the links of the graph.
 * @param info description of the graph.
 */
public void saveGraphMatlab(PrintWriter p, String info) {

        p.println("%%%%%%%%%%%%%%%%\n%"+info+"\n%%%%%%%%%%%%%%%%");
        saveNodeListMatlab(p); 
        saveLinkListMatlab(p);
