...
/**
 * Class Relation. Implements relations among variables in a graphical model.
 *
 * @since 1/12/2005
 */
...
//public static final int PRODUCT = 11;
    ...
    /**
     * Creates a new empty relation. By default the relation created cotains a
    ...
    /**
     * Creates a new relation of type potential from a given
     * <code>Potential</code>. It will have the same variables as the potential
     * and the given potential as values.
     *
    ...
    /**
     * This method is used to get the node child
     *
     * @return node child
     */
    public Node getChild() {
        return getVariables().getNodes().get(0);
    }
    ...
    /**
     * Sets the variables contained in parameter
     * <code>nodes</code> according to the order of this variables in parameter
     * <code>names</code>.
     *
     * @param names The list of names of the variables that are going to be set.
     * @param nodes The list of nodes that contain the variables that are going
     * to be set.
     */
    ...
    /**
     * Sets the variables equal to a clone of the vector of nodes. Note that the
    ...
    public void setKind(String s) {
        ...
        if (s.equals(CONDITIONAL_PROB_STRING)) {
        ...
        } else if (s.equals(UTILITY_STRING)) {
            kind = UTILITY;
        } else if (s.equals(CONSTRAINT_STRING)) {
            kind = CONSTRAINT;
        } else if (s.equals(POTENTIAL_STRING)) {
            kind = POTENTIAL;
        } else if (s.equals("Or")) {
            kind = OR;
            ...
            kind = XOR;
        } else if (s.equals(UTILITY_COMBINATION_STRING)) {
            kind = UTILITY_COMBINATION;
        } /*if (s.equals("Sum"))
         kind = SUM;
    ...
    public String toString() {
        ...
        if (values != null) {
            ...
            str += ") --- global size: " + values.getSize();
        } else {
            str += ") --- real size: " + variables.getSize();
        }
        str += " --- " + type;
    ...
    /**
     * Copies a
     * <code>Relation</code>. Depending on the parameter probabilities, the
     * potential is copied or not
     *
    ...
    /**
     * This method tell if this relation has the same variables that the
     * argument relation.
     *
    ...
    /**
     * This method obtains the union of this relation and the one given as
     * argument. The result of the union is stored in this relation.
     *
    ...
    /**
     * Calculates the intersection between this relation and the argument one.
     *
    ...
    /**
     * Restricts the relation to the variables in the argument list. NOTE:
     * <code>otherValues</code> is set to
     * <code>null</code>.
     *
     ...
     *
     * @return <code>true</code> if the utility node takes part in this
     * relation.
     */
     ...
     *
     * @return <code>true</code> if a continuous variable takes part in this
     * relation.
     */
    ...
    /**
     * Gives the list of non-utility nodes. Gives all the variables of the
     * relation if the node is a super value node.
     *
     * @return the non-utility nodes.
     */
    ...
    public void setHead(NodeList head) {
        this.head = head;
        this.values.setHead(head.toVector());
    }
    ...
    public void setTail(NodeList tail) {
        this.tail = tail;
        this.values.setTail(tail.toVector());
    }
    ...
    public void setHeadAndTail() {
        ...
        if (variables.size() > 0) {
            head.insertNode(variables.elementAt(0));
        }

        this.setHead(head);
        this.getValues().setHead(head.toVector());
        
        NodeList tail = variables.difference(head);
        this.setTail(tail);
        this.getValues().setTail(tail.toVector());
    ...
    public void printHeadAndTail() {
        ...
        if (head != null) {
            for (int i = 0; i < head.size(); i++) {
                strVars += head.elementAt(i).getName() + " ";
            }
        }
        strVars += "|";
        ...
        if (tail != null) {
            for (int i = 0; i < tail.size(); i++) {
                strVars += tail.elementAt(i).getName() + " ";
            }
        }
        System.out.println("pot(" + strVars + ")");
