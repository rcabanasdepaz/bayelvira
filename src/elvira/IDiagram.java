...
/**
 * This class implements the structure for storing and manipulating the
 * Influence Diagrams.
 *
 * @version 0.1
 * @since 21/9/2000 @Autor: Manuel Gomez (with copy&paste of Bnet.java)
 */
public class IDiagram extends Bnet {
    ...
    /**
     * Policies that are forced to some decisions. Evaluation algorithms for IDs
     * don't take into account this attribute to propagate
     */
...
//Array with the utilities of the chance nodes in ArcReversal's method 
    ...
    /**
     * Stores the
     * <code>IDiagram</code> in the file given as parameter.
     *
     * @param f file where the
     * <code>IDiagram</code> is saved.
     * @see Network#save
     */
    ...
    /**
     * Saves the header of the file that will contain this diagram
     *
     * @param p the file.
     */
    ...
    public boolean isFirstConstraint() {
        return firstConstraint;
    }

    public void setFirstConstraint(boolean firstConstraint) {
    ...
    /**
     * Receives a list of variables, from another IDiagram and return a list
     * with the variables in this
     *
     * @param <code>NodeList</code> list of variables
     * @return
     * <code>NodeList</code> list of variables in this
     */
    ...
    /**
     * Receives a list of relations, from another IDiagram, and converts them so
     * the variables are translated to the real set of variables in this
     * IDiagram
     */
    ...
    public boolean areDecisionsOrdered() {
        ...
        /*
         * Node lastDecision; NodeList decisions=getDecisionList(); int
         * numberOfDec=numberOfDecisions();
         *
         * //	Test if all of the decision nodes are inserted
         *
    ...
    /**
     * Checks that there is a path between the decision nodes and randomly order
     * decisions that are not ordered by adding some links
     *
     * @return
     * <code>true</code> if the decisions were ordered previously,
     * <code>false</code> in other case.
     */
     ...
     * decisions that are not ordered by adding some links. It's more efficient
     * than orderDecisionsRandomlyIfNotOrdered. Compare with
    ...
    /**
     * Method to remove informative arcs. This method is required to solve the
     * influenced diagram with a juntion tree
     */
    ...
    /**
     * To remove the barren nodes of the diagram (chance or decision nodes
     * without sucesors).
     */
    ...
    private void removeVariableDoesntMakeSense(Node theNode, Node theParent, double[] theValues, Relation theConstraint) {
        if (theNode.getKindOfNode() == Node.DECISION) {
            ...
            /*
             * Depending on toRemove, there is something to do with the
             * values...
             */
     ...
     *
     * @return
     * <code>true</code> if there is a cycle, or
     * <code>false</code> in other case.
     */
    ...
    public Vector<NodeList> getChancePartitions(){
    
        Vector<NodeList> partitions = new Vector<NodeList>();
                
        NodeList decisions =  getDecisionList();

        // Now, as first step, get all nodes without decisions as sucessors
        NodeList nodeSet = getChanceNodesWithoutDecisionsAsSucessors();
        NodeList newNodeSet;

        nodeSet.printNames();
        partitions.insertElementAt(nodeSet, 0);
        ...
        for (int i = decisions.size() - 1; i >= 0; i--) {
            ...
            newNodeSet = getChanceNodesWithSucessorsInSet(nodeSet);
            newNodeSet.printNames();
            
            partitions.insertElementAt(newNodeSet, 0);
    ...
    /**
     * Method to get the list of nodes with sucessors in a given set of nodes
     */
    ...
    /**
     * To check the instantiation order given by the vector passed as argument
     *
     ...
     * Having several utility nodes without children will be considered in the
     * future.
     *
     ...
     *
     * @return the number of values to store: both probabilities and utilities.
     */
     ...
     *
     * @return the size of the actual set of potentials associated to the
     * relations
     */
    ...
    /**
     * Copy this diagram with relations, but not the values of the relations
     */
    ...
    public void compile(int index, Vector parameters) {
        ...
        switch (index) {
            ...
            case 0:
                Network bVE = iDCopy; // Need to copy: barren nodes are removed
                Evidence eVE;
            ...
            case 1:
                Network bVEPT = iDCopy; // Need to copy: barren nodes removed
                Evidence eVEPT;
                ...
                vePT.setThresholdForPrunning(getFloatElement(parameters, 0)); /*
                 * TO SET
                 */
                //vePT.setThresholdForPruningProb(0.0);
            ...
            case 2:
                Network bVEPTC = iDCopy; // Need to copy: barren nodes removed
                Evidence eVEPTC;
                ...
                vePTC.setThresholdForPruningProb(getFloatElement(parameters, 0));/*
                 * TO SET
                 */
                //vePTC.setThresholdForPruningProb(0.0);
            ...
            case 4:
                ...
                evalPT.setThresholdForPrunning(getFloatElement(parameters, 0));/*
                 * TO SET
                 */
                //evalPT.setThresholdForPruningProb(0.0);
            ...
            case 5:
                ...
                evalPTC.setThresholdForPrunning(getFloatElement(parameters, 0));/*
                 * TO SET
                 */
                //evalPTC.setThresholdForPruningProb(0.0);
     ...
     * possible existence of constraints on it. So will be returned a relation
     * of another kind
     *
    ...
    /**
     * To check the presence of constraints related to this realtion
     *
    ...
    public boolean applyConstraintsOnRelation(Relation rel) {
        ...
        int i;
        double sumAfter, sumBefore;

        

        if (generateDebugInfo) {
            System.out.println("IDiagram:  applyConstraintsOnRelation ----- BEGIN");
        }

        //Retrieve the potential tree of the relation
        potTree = (rel.getValues());
        potTree = applyConstraintsOnPotential(potTree, rel.getKind() == Relation.UTILITY);
        
      /*
        
        
       //         Potential pot = potTree.copy();
                ...
                if (common.size() != 0) {
                    ...
                    if (result == true) {
                        ...
                        if (logexp.getResult() == null) {
                            ...
                            logexp.setPotentialTypeAs(potTree);
                            logexp.evaluate();
                        }
                        ...
                        if (generateDebugInfo) {
                            ...
                            System.out.println("\t - Reestricción:"+logexp+" \n\t"+ constraint);
                        ...
                        //System.out.println("Reestricción");
                        combined = true;
...
*/
        ...
        if (generateDebugInfo) {
            if(!combined)
                System.out.println("\tNo hay reestricciones aplicables");
            System.out.println("IDiagram:  applyConstraintsOnRelation ----- END");
        }
 ...
 /*public boolean applyConstraintsOnRelation(Relation rel) {
    ...
    /**
     * To check the presence of constraints related to a relation The main node
     * to be considered is passed as argument
     *
     * @param <code>Relation</code> the relation to consider
...
*/
    ...
    /**
     * To check the presence of constraints related to this PotentialTree
     *
     * @param <code>PotentialTree</code> the potential to consider
     * @param <code>boolean</code> flag to set if it is an utility potential
     */
    public Potential applyConstraintsOnPotential(Potential pot, boolean utility) {
        LogicalExpression logexp;
        Relation relation;
        NodeList common;
        NodeList varsInPot;
        Potential constraint, combinedConstraints = null;
        Potential finalTree = null;
        boolean result = false;
        boolean combined = false;
        int i, numConstraints=0;
  ...
  //                      System.out.println("~~MAX-MARG:");
  //                      ((PotentialBPTree)constraint).printTikz(0, 0, 0);
                        
                        
 
                        if(combinedConstraints == null)
                            combinedConstraints = constraint;
                        else
                            combinedConstraints = combinedConstraints.combine(constraint);
                        
                        if (generateDebugInfo) {
                            //  constraint.print();
                            System.out.println("\tReestricción aplicable:");
                            System.out.println("\t - Reestricción:" + logexp + " \n\t" + constraint);
                        }
        ...
        if (combinedConstraints != null) {
            if (generateDebugInfo) {
                System.out.println("\tAplicando reestricción:");
                System.out.println("\t - Potencial: " + finalTree);
                System.out.println("\t - Reestricción: " + combinedConstraints);
            }


  //           System.out.println("~~COMB-CONSTR:");
  //          ((PotentialBPTree)combinedConstraints).printTikz(0, 0, 0);        
 
            
            
            finalTree.limitBound(0.0);           
            BinaryProbabilityTree.setImprovedOps(true);
           
            if (firstConstraint) {
                finalTree = combinedConstraints.combine(finalTree);
            } else {
                finalTree = finalTree.combine(combinedConstraints);
            }
            BinaryProbabilityTree.setImprovedOps(true);
           
  //          System.out.println("~~COMB-CONSTR-POT:");
  //          ((PotentialBPTree)finalTree).printTikz(0, 0, 0);      
            
            
            finalTree.setHead(pot.getHead());
            finalTree.setTail(pot.getTail());
            ...
            if(prop != null)
                prop.getStatistics().addNumConstraints(numConstraints);
            
            if (finalTree.getNumberOfNodes() < pot.getNumberOfNodes()) {
                prop.getStatistics().addNumAppliedConstraints(numConstraints);
                if(generateDebugInfo)
                    System.out.println("\t - Sí se aplica la reestricción :"+pot.getNumberOfNodes()+" -> "+finalTree.getNumberOfNodes());
                
            }else if(generateDebugInfo) {
                System.out.println("\t - Se descarta aplicar la reestricción :"+pot.getNumberOfNodes()+" -> "+finalTree.getNumberOfNodes());
            }

            if (generateDebugInfo) {
                System.out.println("\t - Resultado tras combinar: " + finalTree);
            }
        }
 ...
 /*public Potential applyConstraintsOnPotential(Potential pot, boolean utility) {
            ...
            if (relation.getKind() == Relation.CONSTRAINT) {
                ...
                if (common.size() != 0) {
                    ...
                    if (result == true) {
                        ...
                        if(firstConstraint) 
                            finalTree = constraint.combine(finalTree);
                        else
                            finalTree = finalTree.combine(constraint);
        ...
        prop.getStatistics().addNumConstraints(numConstraints);
        if(finalTree.getSize()<pot.getSize()) {
            prop.getStatistics().addNumAppliedConstraints(numConstraints);
        }
...
*/
    /**
     * Checks if a given potential is a conditional probability distribution for
     * a given variable
     *
    ...
    /**
     * Method to prepare the normalization of the potential linked to a relation
     *
     * @param <code>String</code> main node of the relation
     * @param <code>PotentialTree</code> Potential to be normalized
     */
    public void normalize(Node node, Potential pot) {
        if(true)return;
        System.out.println("NORMALIZE BEGIN");
     ...
     //   Potential pot0 = pot.copy();
        Potential divisor = pot.addVariable(node);
        //       Potential divisor2 = pot.addVariable(node);
        pot = pot.divide(divisor);


      //  pot.repair(conditioning);

/*System.out.println("pot0, pot1 "+EUComparator.compareUtilities(pot0, pot1));
System.out.println("pot0, pot "+EUComparator.compareUtilities(pot0, pot));
System.out.println("pot, pot1 "+EUComparator.compareUtilities(pot, pot1));
* 
*
System.out.println("-----------");
...
//    pot.print();
    ...
    /**
     * This function is used to know whether the link that we want to add, and
     * whose head is 'child' and whose tail is 'father' is compatible with the
     ...
     * @return true iff the link is compatible with the structure of influence
     * diagram
     */
...
//	Compute the rest of levels
   ...
   public Vector<NodeList> getPartialOrder(){
    
        Vector<NodeList> p = new Vector<NodeList>();
        
        NodeList decisions = this.getDecisionList();

        // Now, as first step, get all nodes without decisions as sucessors
        NodeList nodeSet = this.getChanceNodesWithoutDecisionsAsSucessors();
        NodeList newNodeSet = this.getChanceNodesWithoutDecisionsAsSucessors();
        p.add(newNodeSet);
        ...
        for (int i = decisions.size() - 1; i >= 0; i--) {
            ...
            newNodeSet = new NodeList();
            newNodeSet.insertNode(decisionConsidered);
            p.add(newNodeSet);
            
            nodeSet.insertNode(decisionConsidered);
            
            // Now, get all chance nodes with sucessors in phase
            newNodeSet = this.getChanceNodesWithSucessorsInSet(nodeSet);
            p.add(newNodeSet);
    ...
    /**
     * Method for computing the number of configurations constrained by a given
     * constraint
     *
     * @param constraint
     * @return constrained
    ...
    public Propagation getProp() {
        return prop;
    }

    public void setProp(Propagation prop) {
        this.prop = prop;
    }
    
    
    public void combineMultipleUtilities() {
        
        try {
            
            NodeList utils = this.getNodesOfKind(Node.UTILITY);
            String newName = "";
            Potential newPot = null;
            for(int i=0; i<utils.size(); i++) {
                Node n = utils.elementAt(i);
                Potential p = this.getRelation(n).getValues();
                this.removeRelation(n);
                ...
                if(newPot==null){
                    ...
                    newPot = newPot.addition(p);
                }


                if(i!=0)
                    newName += "_";
                newName += n.getName().replaceAll("\n",""); 

            }

            NodeList newParents = newPot.getNodeList();
            Continuous newNode = new Continuous(newName);
            newNode.setKindOfNode(Node.UTILITY);
            newNode.setName(newName);
            newNode.setTitle(newName);

            //Sets the parents
            LinkList newParentLinks = new LinkList();
            for(int i=0; i<newParents.size(); i++) {
                Link link = new Link(newParents.elementAt(i),newNode);
                newParentLinks.insertLink(link);
                newParents.elementAt(i).addChild(link);
                this.addLink(link);
                
            }

            newNode.setParents(newParentLinks);
            Relation newRel = new Relation(newNode);
            newRel.setValues(newPot);
            newRel.setName(newName);
            
            Vector vars = newParents.getNodes();
            vars.insertElementAt(newNode, 0);
            newRel.setVariables(vars);
            
            
            
            
            newRel.setKind(Relation.UTILITY);
            this.addRelation(newRel);
           
            this.addNode(newNode);
            
        } catch (InvalidEditException ex) {          
            Logger.getLogger(IDiagram.class.getName()).log(Level.SEVERE, null, ex);
