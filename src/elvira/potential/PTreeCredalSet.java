...
public class PTreeCredalSet extends elvira.potential.PotentialTree implements CredalSet{
  ...
  public PTreeCredalSet(PotentialInterval intervalSet){
    ...
    if(getVariables().size()==1) {
        ...
        //Creates a subtree for each configuration of the parents

        Configuration confTail = new Configuration(intervalSet.getTail());

        PotentialInterval subPot = null;

        for (int y = 0; y < confTail.possibleValues(); y++) {

            subPot = (PotentialInterval) intervalSet.restrictVariable(confTail);
            subPot.print();
            
            PTreeCredalSet subTree = new PTreeCredalSet(subPot);
           
   //         subTree.print();
            
            this.setSubTree(confTail, new PTreeCredalSet(subPot));
