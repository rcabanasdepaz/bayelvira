 ...
 */
    ...
    public PotentialTable(Vector vars, double defaultValue) {
...
/**
 * Gets the values of the potential.
 * @return a vector of <code>double</code> with the values of the potential.
 */

public Vector<Double> getVectorValues() {
    Vector v = new Vector();
...
public String toString2D(int varsX, int varsY, String sepCell, String sepLine) {
...
public PotentialTable addVariable(Vector vars) {
  ...
  for (i=0 ; i<values.length ; i++) {
    ...
    if(!useBigDecimal)
        ...
        pot.values[pos] = BigDecimal.valueOf(pot.values[pos]).add(BigDecimal.valueOf(values[i])).doubleValue(); 
    numSum++;
...
public Potential combine(Potential p) {
  ...
  if ((p.getClass()==PotentialTable.class) ||
    ...
    for (i=0 ; i<pot.values.length ; i++) {
      ...
      if(!useBigDecimal)
        ...
        x = BigDecimal.valueOf(x).multiply(BigDecimal.valueOf(p.getValue(conf2))).doubleValue(); 
  ...
  }else if(p.getClass()==PotentialIntervalTable.class){
      return p.combine(this);
...
public PotentialTable Add(PotentialTable p) {
  ...
  double x;
      ...
      //Qualitative operations
    if (isQualitativeOps()) {
        Vector newVars = VectorManipulator.union(this.getVariables(), p.getVariables());
        PotentialTable res = new PotentialTable(newVars);
        return res;
    }
  ...
  for (i=0 ; i<pot.values.length ; i++) {
    ...
    if(!useBigDecimal)
        ...
        x = BigDecimal.valueOf(x).add(BigDecimal.valueOf(p.getValue(conf2))).doubleValue(); 
    pot.setValue(conf,x);
    conf.nextConfiguration();
  }
...
public static int getIndexInTable(int[] values, double[] weights){
  ...
  public Potential sendVarToEnd(Node x) {
    ...
    return((Potential)newPotential);
      ...
      /**
     * Computes the addition of all the values
    ...
    public boolean isUnity() {
        ...
        for(int i=0; i<values.length; i++){
            if(Math.abs(values[i]-1.0)>0.00000001)
                return false;
    ...
    public static PotentialTable getOnes(Vector vars) {
    ...
    public static void setUseBigDecimal(boolean useBigDecimal) {
        PotentialTable.useBigDecimal = useBigDecimal;
    }
