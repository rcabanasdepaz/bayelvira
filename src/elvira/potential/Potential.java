...
public abstract class Potential implements Cloneable, Serializable {
   ...
   protected Vector head;
   protected Vector tail;
   
   

   protected static boolean generateStatistics = false;
   
   private static PropagationStatistics statistics = null;
   
   private static boolean qualitativeOps = false;
   ...
   public long getSize() {
    ...
    public void normalizeOver(FiniteStates v) {
      System.out.println("normalizeOver(FiniteStates v): this method must be overloaded in " + this.getClass());
      System.exit(1);
    }
       /**
    ...
    public double sum(long treeSize) {
      System.out.println("Error in Potential.sum(): this method must be overloaded in " + this.getClass());
      System.exit(1);
      return 0;
   ...
   public Vector getVariables() {
      ...
      /**
    * Gets the variables of the potential.
    * @return a <code>NodeList</code> of objects of class
    * <code>Node</code>
    * with the variables of the potential.
    */
   public NodeList getNodeList() {
      return new NodeList(variables);
   }
   ...
   public Potential setVariablesOrder(Node[] order){
       ...
       for (int i=0; i<order.length; i++)
           pot = (PotentialTable) pot.sendVarToEnd(order[i]);
...
//       System.out.println("WARNING: At the moment the method argMax is not implemented for " + this.getClass().getName());
      return null;
       
   }
   
      /**
    * Marginalizes over a  variable using MAXIMUM as marginalization
    * operator.
    * @param vars a <code>Vector</code> of variables (<code>Node</code>).
    ...
    *
    */
   public Potential maxMarginalizePotential(Node var) {
       Vector vars = new Vector();
       vars.add(var);
       return maxMarginalizePotential(vars);
   }
   
   
   
         /**
    * Marginalizes out a  variable using MAXIMUM as marginalization
    * operator.
    * @param vars a <code>Vector</code> of variables (<code>Node</code>).
    * @return a <code>Potential</code> with the marginalization over
    * <code>vars</code>.
    ...
    *
    */
   public Potential maxVariable(Node var) {
       Vector vars = new Vector();
       
       for(int i=0; i<variables.size(); i++) {
           if(!variables.elementAt(i).equals(var))
   ...
   public String getClassName() {
    ...
    public static boolean isQualitativeOps() {
        return qualitativeOps;
    }

    public static void setQualitativeOps(boolean qualitativeOps) {
        Potential.qualitativeOps = qualitativeOps;
    }
       ...
       if(variables!=null)
         ...
         if (head == null || tail == null) {
             ...
             for(int i=0; i<head.size(); i++) {
                 node = (Node) head.elementAt(i);
                 s += (" " + node.getName());
             }
             if(tail.size()>0)
                 s+=" |";
             for(int i=0; i<tail.size(); i++) {
                 node = (Node) tail.elementAt(i);
                 s += (" " + node.getName());
             }
   ...
   s +=")";
     ...
     * @return <code>Potential</code> Potential already normalized
     */
    
    public void repair(NodeList conditioning) {
    ...
    public long getNumConfigurations(){
        ...
        for(int i=0; i<getVariables().size(); i++){
            num*= ((FiniteStates)getVariables().get(i)).getNumStates();
        }
    ...
    public void setFirstVarAsHead() {
        
        setHead((Node)variables.elementAt(0));
        
        for(int i=1; i<variables.size(); i++) {
            tail.add(variables.elementAt(i));
        }
    }
    ...
    public static Vector getCombinationHead(Potential pot1, Potential pot2){
        ...
        Vector h1 = pot1.getHead();
        Vector h2 = pot2.getHead();
        
        if(h2==null || h1==null)
            System.out.println();
        
        return VectorManipulator.union(h1, h2);
  
    }
   
   
    public static Vector getCombinationTail(Potential pot1, Potential pot2){
        ...
        if (pot2 == null) {
            ...
            Vector h12 = Potential.getCombinationHead(pot1, pot2);
            
            Vector t1 = pot1.getTail();
            Vector t2 = pot2.getTail();
            Vector t12 = VectorManipulator.union(t1, t2);
            t12.removeAll(h12);
            return t12;
    ...
    public static Vector getMargHead(Potential pot, Vector removedVars) {
        
        Vector newHead = new Vector();
        
       
        newHead.addAll(pot.getHead());
        newHead.removeAll(removedVars);
    ...
    public static Vector getMargTail(Potential pot, Vector removedVars) {
        Vector newTail = new Vector();
        newTail.addAll(pot.getTail());
        newTail.removeAll(removedVars);
     ...
     public static Vector getMargHead(Potential pot, Node removedVar) {
        
        Vector newHead = new Vector();
        newHead.addAll(pot.getHead());
        newHead.remove(removedVar);
    ...
    public static Vector getMargTail(Potential pot, Node removedVar) {
        Vector newTail = new Vector();
        newTail.addAll(pot.getTail());
        newTail.remove(removedVar);
        
        return newTail;
        
    } 
    
   public static boolean generatesUnityPot(Potential pot, Node n) {
    ...
    public static boolean generatesUnityPot(Potential pot, NodeList removedVars ) {
        ...
        for(int i=0; i<removedVars.size(); i++) {
            Node n = removedVars.elementAt(i);
            if(head.contains(n))
                head.remove(n);
        }
    ...
    public static boolean generatesUnityPot(Potential pot1, Potential pot2, NodeList removedVars ) {
        Vector head = new Vector();
        head.addAll(Potential.getCombinationHead(pot1, pot2));
    ...
    public static PropagationStatistics getStatistics() {
        return statistics;
    }

    public static void setStatistics(PropagationStatistics statistics) {
        generateStatistics = true;
        Potential.statistics = statistics;
    }
   ...
   /**
    ...
    public boolean isNonNegative() {
      System.out.println("isNonNegative: Not implemented in this class");
      System.exit(1);
      return false;
    }
