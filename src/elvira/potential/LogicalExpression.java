 ...
 */
private Potential result;


/**
 * To store the type of tree encoding the logical expresion (PotentialTree,
 * PotentialBPTree, etc.)
 */
private String potentialType = "elvira.potential.PotentialTree";


private static boolean sortStates = false;
...
public void setConsecuent(LogicalNode consecuent){
  this.consecuent=consecuent;
    ...
    public void setPotentialTypeAs(Potential pot) {
        if(pot instanceof PotentialBPTree)
            this.potentialType = "elvira.potential.binaryprobabilitytree.PotentialBPTree";
        else 
            this.potentialType = "elvira.potential.PotentialTree";
    }
...
public void setResult(Potential result){
  this.result=result;
}
...
public void evaluate(){
  ...
  // Now transform it to a tree
  if(potentialType.equals("elvira.potential.PotentialTree")){
    result=resultTable.toTree();
    ((PotentialTree)result).sortAndBound(0L);
  }else if(potentialType.equals("elvira.potential.binaryprobabilitytree.PotentialBPTree")){
      result = new PotentialBPTree(resultTable);
      

      ((PotentialBPTree)result).sort(sortStates);
      ((PotentialBPTree)result).limitBound(0L);
...
public LogicalNode getConsecuent()
{
    ...
    public static boolean isSortStates() {
        return sortStates;
    }

    public static void setSortStates(boolean sortStates) {
