...
public class ProbabilityTree{
    ...
    public void insertChild(ProbabilityTree tree) {
     ...
     /**
     * Set a <code>ProbabilityTree</code> as the child of the state <code>s</code>.
     * @param tree <code>ProbabilityTree</code> to be added as child.
     */
    public void setChild(ProbabilityTree tree, int s) {
        child.set(s, tree);
    }
    ...
    ProbabilityTree tree2) {
        ...
        if (improvedOps) {
            ...
            if ((tree1.isProbab() && tree1.value == 0) || (tree2.isProbab() && tree2.value == 0)) {
                tree = new ProbabilityTree(0);
                return tree;
            }
            //If one of the trees is a leaf with the value 1,
            //the result will be the other tree
            if ((tree1.isProbab() && tree1.value == 0)) {
                tree = tree2.copy();
                return tree;
            }
            ...
            //If one of the trees is a leaf with the value 0,
            //the result will be always 0
            if ((tree2.isProbab() && tree2.value == 0)) {
                tree = tree1.copy();
                return tree;
    ...
    ProbabilityTree tree2) {
        ...
        if(improvedOps){
            ...
            if((tree1.isProbab() && tree1.value==0)||(tree2.isProbab() && tree2.value==0)){
             ...
             //If tree2 is a leaf with the value 1,
            //the result will be the other tree
            if((tree2.isProbab() && tree2.value==1)){
                tree = tree2.copy();
                return tree;
...
/*        if(improvedOps) {            
            ...
            //If one of the trees is a leaf with the value 0,
            //the result will be the other tree
            if(this.isProbab() && this.value==0) {
        //        System.out.println("improvedOps sum");
                return tree.copy();
            }
            if(tree.isProbab() && tree.value==0) {
         //       System.out.println("improvedOps sum");
                return this.copy();
            }
        }
  ...
  */      
    ...
    long numberDeleted[]) {
        ...
        if (bounded) {
            ...
            if(Math.abs(info)<IDVEWithPotentialTree.limitForPruning)
                info=0;
    ...
    public boolean pruneUtility_topDown(double limit,double minG, double maxG, long[] numberDeleted) {
        int i, numberChildren;
        ProbabilityTree ch;
        double finalLimit;
        boolean bounded = true; // indicates whether the tree can be pruned
        ...
        if(Math.abs(info)<IDVEWithPotentialTree.limitForPruning)
          ...
          if(info<=finalLimit) {   //The tree is prunned        
                ...
                for(i=0; i < numberChildren; i++){
                    ch=getChild(i);
                    ch.pruneUtility_topDown(limit, minG, maxG, numberDeleted);
                }
                bounded = false;
      ...
      public ProbabilityTree getSortedUtility(long oldSize, NodeList variables) {
        ...
        Vector<ProbabilityTree> bestChl = null;
        
        ProbabilityTree outTree;
        ...
        FiniteStates candidateVar;
        for(int j=0; j<variables.size(); j++) {
            candidateVar = (FiniteStates) variables.getNodes().get(j);
            // Get the number of childs of this node  
            numberChildren = candidateVar.getNumStates();
            
            
            //Expands the tree once and computes the distance
            Vector<ProbabilityTree> candidateChl = new Vector<ProbabilityTree>();
            Vector vleaves = new Vector();
            Vector vmean = new Vector();
            Vector factors = new Vector();
            for(i=0; i < numberChildren; i++){

                ch=this.restrict(candidateVar, i);
                candidateChl.add(ch);               

                Vector chLeaves = ch.getLeaves();
                vleaves.addAll(chLeaves);
                Vector chFactors = ch.getFactors(oldSize/numberChildren);
                factors.addAll(chFactors);

                vmean.addAll(VectorManipulator.vectorMean(chLeaves, chFactors));
            }
            try {
                d2 = Distances.euclidean(vmean, vleaves, factors);

                //Computes the distance without expanding
                vmean = VectorManipulator.vectorMean(vleaves, factors);
                d1 = Distances.euclidean(vmean, vleaves, factors);


            } catch (Exception ex) {
                Logger.getLogger(ProbabilityTree.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            //Checks if it is the most informative variable
            if(info>maxInfo) {
                maxInfo = info;
                bestVar = candidateVar;
                bestChl = candidateChl;
        ...
        //Build the node
        outTree = new ProbabilityTree(bestVar);
        numberChildren = bestVar.getNumStates();
        long chSize = oldSize/numberChildren;
        NodeList chVars = variables.copy();
        chVars.removeNode(bestVar);

        //Sort the children
        for(i=0; i<numberChildren; i++){

            outTree.setChild(bestChl.get(i).getSortedUtility(chSize, chVars), i);
        }
    ...
    public boolean pruneUtility_downTop( double finalLimit, long oldSize) {
        int i, numberChildren;
        ProbabilityTree ch;
        double max=0,min=0;
        double pr;
        ...
        //Checks children are leaves, if not, tries to prune them
        for(i=0; i < numberChildren; i++){
            ch=getChild(i);
            ...
            if (ch.label != PROBAB_NODE){
                ...
                // subtree
                childBounded = ch.pruneUtility_downTop(finalLimit, oldSize/numberChildren);
        ...
        //If we are situated at a terminal tree
        if(bounded){
            ...
            mean = mean/numberChildren;

            //Computes the distance
            for(i=0; i < numberChildren; i++){
                ch = getChild(i);
                info += chSize* Math.pow(ch.getProb() - mean, 2);
            }
    ...
    public long getSize() {
        ...
        if (label == PROBAB_NODE) {
            num = 1;
        }else if(label == FULL_NODE){
            
            
            int nv = var.getNumStates();
            for (int i = 0; i < nv; i++) {
                num += ((ProbabilityTree)child.get(i)).getSize();
            }
    ...
    public String printTikz(double posX, double posY, int level, int initState) { 
        ...
        if(!isProbab()) {
            if(var.getKindOfNode() == Node.CHANCE) {
                nodeType = "[chance]";
            }else if(var.getKindOfNode() == Node.DECISION) {
                nodeType = "[decision]";
            }else if(var.getKindOfNode() == Node.UTILITY) {
                nodeType = "[util]";
            }
        }
        ...
        if(level==1)
            s += offset+"\\node"+nodeType+" at ("+posX+", "+posY+") ";
        else
            s += offset+"node"+nodeType+" ";
        ...
        if (label == PROBAB_NODE) {
            ...
            for(i=0 ; i< child.size() ; i++) {
                ...
                ProbabilityTree ch = ((ProbabilityTree)child.elementAt(i));
            
                s += offset+"child{\n";
                s += ch.printTikz(0, 0, level+1, initState);
...
public int isPropApprox(double prI,double prJ,Vector vProbs,Vector vfacK,
  ...
  return (indexFac); // returns 0    (-1 if not proportional)
    ...
    public Vector getFactors(long oldSize) {
        Vector factors = new Vector();
        this.privateGetFactors(factors, oldSize);
        return factors;
    }


    private void privateGetFactors(Vector factors, long oldSize) {
        ...
        if (label == PROBAB_NODE) {
            ...
            int nv = var.getNumStates();
            for (int i = 0; i < nv; i++) {
                ((ProbabilityTree)child.get(i)).privateGetFactors(factors, oldSize/nv);
            }
    ...
    public int getNumConfigurationsWithVariable(Node n) {
        ...
        if(n.equals(var))
            ...
            num += getChild(i).getNumConfigurationsWithVariable(n);
    ...
    public long getNumPrunedConf(long totalSize, Node n) {
        ...
        if(n.equals(var))
            ...
            int numChildren = this.child.size();
            for(int i=0; i<numChildren; i++) {
                num += getChild(i).getNumPrunedConf(totalSize/numChildren, n);
    ...
    public static void setImprovedOps(boolean improvedOps) {
        ProbabilityTree.improvedOps = improvedOps;
    }
    ...
    public void replaceLowValues(double threshold, boolean absValue) {
      if(isProbab()){
          ...
          for(int i=0; i<child.size();i++) {
                ((ProbabilityTree)child.elementAt(i)).replaceLowValues(threshold, absValue);
          }
      }                  
    ...
    public double[] getMaxMin() {
        ...
        if(isProbab()) {
            ...
            for(int i=0; i<child.size(); i++) {
               double maxMinChild[] = ((ProbabilityTree)child.elementAt(i)).getMaxMin();
               if(maxMin[0]<maxMinChild[0])
                   maxMin[0] = maxMinChild[0];
               if(maxMin[1]>maxMinChild[1])
                   maxMin[1] = maxMinChild[1];
            }
