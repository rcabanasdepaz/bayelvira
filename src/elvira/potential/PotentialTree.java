...
/*public PotentialTree combine(PotentialTree p) {
  return (PotentialTree)combine((Potential)p);
    ...
    public PotentialTree sortAndBound(double threshold) {
      ...
      public PotentialTree prune(double threshold) {
        PotentialTree pot;
    ...
    public PotentialTree sortUtility() {
        ...
        ProbabilityTree sortedUtility = values.getSortedUtility(this.getNumConfigurations(), new NodeList(this.getVariables()));
        PotentialTree sortedPot = new PotentialTree(this.getVariables());
        sortedPot.setTree(sortedUtility);
    ...
    double limit) {
       ...
       public PotentialTree pruneUtility(double minimum, double maximum,
    double limit) {
        PotentialTree potTree;
    ...
    public void limitBoundUtility(double minimum, double maximum, double limit) {
        ...
        double newLimit = (maximum-minimum)*limit;

        // If the tree is not just a probability value:
        if (!values.isProbab()) {
            values.pruneUtility_downTop(newLimit, this.getNumConfigurations());
            updateSize();
        }
        //size -= numberDeleted[0];
    }
 ...
 *         and the core tree. 
 *@return an empty vector if the variableï¿½s not found, or it's in the root node.
 *              
 */
 ...
 *@return an empty vector if the variableï¿½s not found, or any factorisation have
 *                      been made.
 *              
 */
...
public Vector splitAndFactorisePT(FiniteStates varFac, FactorisationTools factParam){
  ...
  return listofPT;
    ...
    public long getNumPrunedConf(Node n) {
        return this.values.getNumPrunedConf(this.getNumConfigurations(), n);
     ...
     * @param conf a <code>Configuration</code>.
     * @param x a <code>PotentialTree</code>, the new subtree for <code>conf</code>.
     */
    
    public void setSubTree(Configuration conf, PotentialTree x) {
        
        variables = VectorManipulator.union(variables, x.variables);
        values.setSubTree(conf, x.values);
...
//        Vector<Configuration> sample = new Vector<Configuration>();
        ...
        // PRIMER EXPERIMENTO: Generar ÃÂ¡rboles binarios al azar, y factorizar por
        // cada una de las variables, midiendo las medidas de factorizaciÃÂ³n
...
//        }
        ...
        // SEGUNDO EXPERIMENTO: Generar ÃÂ¡rboles ternarios al azar, y factorizar por
        // cada una de las variables, midiendo las medidas de factorizaciÃÂ³n
        // y la log-verosimilitud.
...
//        }
        ...
        // TERCER EXPERIMENTO: Generar ÃÂ¡rboles ternarios al azar, y factorizar por
        // la mejor variable. Luego, los factores descomponerlos tambiÃÂ©n por
        // la mejor variable, y asÃÂ­ sucesivamente, midiendo verosimilitud
        // en cada paso.
//        x1 = new FiniteStates(2);
//        x1.setName("X1");
//        x2 = new FiniteStates(2);
