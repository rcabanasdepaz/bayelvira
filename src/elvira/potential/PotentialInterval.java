 ...
 */
public abstract class PotentialInterval extends Potential implements IPotentialInterval {
  ...
  // compruebainterv_list_extrems in de Campos algorithm
  {
    ...
    /**
   * Gets the extrems points from the set of intervals in this intervalSet. The potential
   * intervalSet must contain only one FiniteState variable in the head but any in the tail
   * @return a Vector of Vectors with the extrem points. Each component in the inner Vector is an array of 
   * double values (one double for each state of the FiniteState variable).
   */
  public Vector getListOfListExtrems() {
        ...
        PotentialInterval subPot = null;
        
        for (int y = 0; y < confTail.possibleValues(); y++) {
        
            subPot = (PotentialInterval) restrictVariable(confTail);
            subPot.print();
            
            listOfLists.add(subPot.getListExtrems());
    ...
    public boolean isIncluded(IPotentialInterval ipot) {
        return isIncluded(ipot, 0.0);
    }

    public boolean isIncluded(IPotentialInterval ipot, double error) {

        
        PotentialInterval pot2 = ((PotentialInterval)ipot);
        
        //Check variables
        Vector vars1 = this.getListNonTransparents();
        Vector vars2 = pot2.getListNonTransparents();
        
        for(int i=0;i<vars2.size(); i++) {
            if(!vars1.contains(vars2.elementAt(i))) {
                System.out.println("ERROR: potentials with different variabels");
                System.exit(1);
            }                
        }
        ...
        for (int i = 0; i < conf.possibleValues(); i++) {
            ...
            if( getMaxValue(conf) > pot2.getMaxValue(conf) || pot2.getMinValue(conf) > getMinValue(conf)){
                if (getMaxValue(conf) - pot2.getMaxValue(conf) > error || pot2.getMinValue(conf) - getMinValue(conf) > error) {
                    System.out.println("[" + getMinValue(conf) + ", " + getMaxValue(conf) + "]" + " [" + pot2.getMinValue(conf) + ", " + pot2.getMaxValue(conf) + "]");
                    included = false;
                }
            }
        }
    ...
    public boolean isPrecIncluded(Potential pot, double error) {
        
        PotentialInterval ipot = null;
        
        if(pot instanceof PotentialInterval) {
            ipot = (PotentialInterval) pot;
        }else{
            ipot = new PotentialIntervalTable(new PotentialTable(pot), 0.0, false);
        }
        
        
        return isIncluded(ipot, error);
    
    
    }
    
    
    public static boolean areIncluded(Vector<Potential> pots, Vector<Potential> ipots) {
    
        boolean included = true;
    
        if(ipots.size() != pots.size()) {
            System.out.println("ERROR in PotentialInterval.areIncluded different sizes of vectors");
            return false;
        }
        
        for(int i=0; i<ipots.size(); i++) {
            PotentialInterval ipot = (PotentialInterval) ipots.elementAt(i);
            Potential pot = pots.elementAt(i);
            
            System.out.println("Checking potentials:");
            ipot.printDomain();
            pot.printDomain();
            
            if(!ipot.isPrecIncluded(pot, 0.0001)) {
                included = false;
                System.err.println("Precise potential "+i+" is not included: ");
                ipot.printDomain();
                pot.printDomain();
        ...
        if(included)
            System.err.println("All the potentials are included");
