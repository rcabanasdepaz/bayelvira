...
public class Distances {
    ...
    public static double euclidean(Vector<Double> v1, Vector<Double> v2) throws Exception { 
       if(v1.size() != v2.size())
            throw new Exception("Euclidean: Vector of different sizes");
     ...
     * @param v1
     * @param v2
     * @param f factors (weigth of each value in the vectors)
     * @return
     * @throws Exception 
     */
    public static double euclidean(Vector<Double> v1, Vector<Double> v2, Vector<Integer> f) throws Exception { 
        if(v1.size() != v2.size())
            throw new Exception("Euclidean: Vector of different sizes");
        return VectorManipulator.euclideanNorm(VectorManipulator.subtraction(v1, v2), f);
 ...
 //       VectorManipulator.print(v2);
    ...
    */    
        double norm1 = VectorManipulator.sumValues(VectorManipulator.pow(v1, 2));
        double norm2 = VectorManipulator.sumValues(VectorManipulator.pow(v2, 2));
        
        
        double numer = VectorManipulator.multiply(v1, v2);
      //  double denom = Math.pow(norm1, 2) +  Math.pow(norm2,2) - numer;
        double denom = norm1 + norm2  - numer;
     ...
     * @throws Exception 
     */
    public static double kullbackLeibler(Vector<Double> v1, Vector<Double> v2, Vector<Integer> f) throws Exception{
        ...
        for(int i=0; i<v1.size(); i++) {            
            if(v1.get(i) != v2.get(i)) {
                for(int j=0; j<f.get(i).intValue(); j++)
                    dist+= v1.get(i)*Math.log(v1.get(i)/v2.get(i));
            }
            //System.out.println("d("+v1.get(i)+","+v2.get(i)+") = "+v1.get(i)*Math.log(v1.get(i)/v2.get(i))+"\t"+dist);
    ...
    public static double distanciaRelativa2(Vector<Double> v1, Vector<Double> v2) throws Exception{
        ...
        /**
     * Computes the percent error between an original vector v2 and its aproximation v1
     * @param v1 aproximation
     * @param v2 original vector
     * @return
     * @throws Exception 
     */
    public static double percentError(Vector<Double> v1, Vector<Double> v2) throws Exception{
        ...
        for(int i=0; i<v1.size(); i++) {
            if((v2.get(i) + v2.get(i))==0)
                ...
                dist+= Math.abs((v1.get(i) - v2.get(i))/(v2.get(i) + v2.get(i)))*2;
