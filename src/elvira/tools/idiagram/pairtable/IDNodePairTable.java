...
public double computeCost(int criteria){
  ...
  switch(criteria){
    case IDPairTable.MINSIZE: //Es igual que CLIQUEWEIGHT (Scw)
       cost=totalSize();
       break;
    case IDPairTable.CANOMORAL:  //Scm
       if (variable instanceof FiniteStates)
          cost=(double)cliqueWeight()/((FiniteStates)variable).getNumStates();
       else
          cost=totalSize();
       break;
    case IDPairTable.CLIQUESIZE : //Scs
       cost=cliqueSize();
       break;
    case IDPairTable.CLIQUEWEIGHT : //Scw
       cost=cliqueWeight();
       break;
    case IDPairTable.MINFILL:   //Sfi
       cost=numberOfLinksToAdd();
       break;
    case IDPairTable.WEIGHTFILL: //Sfiw
       cost=weightOfLinksToAdd();
       break;
    case IDPairTable.WEIGHTFILL_STATES: //Sfiws
       cost=weightOfLinksToAdd()/((FiniteStates)variable).getNumStates();
       break;
    case IDPairTable.MINFILL_STATES: //Sfis
       cost=(double)numberOfLinksToAdd()/((FiniteStates)variable).getNumStates();
       break;
    case IDPairTable.MINFILL_MINSIZE:  //Scwfi
       cost=totalSize()+numberOfLinksToAdd();
       break;
    case IDPairTable.CANOMORAL_MINFILL: //Scmfi
       cost=(double)totalSize()/((FiniteStates)variable).getNumStates()+numberOfLinksToAdd();
       break;
    case IDPairTable.RELATIONSWEIGHT: cost=computeRelationsWeight();  //Srw
       break;
    case IDPairTable.OTRO: cost=(double)((FiniteStates)variable).getNumStates() / totalSize(); //Sicm
       break;
    case IDPairTable.PROD_STATES_IN_TREE:
        cost= (double) totalTreeSize();
        break;
    case IDPairTable.DIFF_PRUNE_DEGREE:
        cost = (double) this.meanDiffPrune();
        break;
   
    case IDPairTable.MIN_RATIO:
        cost = (double)  meanCompressionRatio();
    break;
        
    case IDPairTable.MINLEAVES:
       // double cost2 = (double) totalSize() * meanCompressionRatio();
        cost = (double) totalTreeSize2();
    break;
    case IDPairTable.MINLEAVES_STATES:
        cost = (double) (totalSize() * meanCompressionRatio())/((FiniteStates)variable).getNumStates();
    break;
    case IDPairTable.MINLEAVES_NUMPOTS:
        cost = (double) totalSize() * meanCompressionRatio() * this.getRelationsSize();
    break;    
    
    case IDPairTable.MINTREE:
        cost = (double) totalNodesInTree();
        break;
    case IDPairTable.MAXTREE:
        cost = (double) 1.0/totalNodesInTree();
        break;
    case IDPairTable.MINTREE_STATES:
        cost = (double) totalNodesInTree()/((FiniteStates)variable).getNumStates();

    break;
    case IDPairTable.MINTREE_PROB_UTIL:
        cost = (double) totalNodesInTree() + (double) totalNodesInProbTree();
    break; 
        
    case IDPairTable.MINTREE2:
        cost = (double) totalNodesInTree(true);
    break;    

    case IDPairTable.MINTREE_STATES2:
        cost = (double) totalNodesInTree(true)/((FiniteStates)variable).getNumStates();

    break;
    case IDPairTable.MAXCLIQUESIZE : //Scs
       cost=1/cliqueSize();
       break;
    case IDPairTable.MAXWEIGHT : //Scw
       cost=1/cliqueWeight();
       break;
