...
/**
 * Class
 * <code>RelationList</code>. Implements a list of relations.
 *
 * @since 22/9/2000
 */
public class RelationList {
    ...
    * <code>rel</code> in the list. -1 if
    * <code>rel</code> is not in it.
    */
    ...
    * @return the relation at position
    * <code>p</code>.
    */
   ...
   /**
    * Computes the size of the potential resulting from the combination of all
    * the relations in the list, considering fully expanded potentials and
   ...
   /**
    * Computes the size of the potential resulting from the combination of all
    * the relations in the list that contain the argument variable, considering
    * fully expanded potentials and
    * <code>FiniteStates</code> variables.
    *
   ...
   /**
    * Method to compute the total size needed to store the values used to
    * quantify the whole set of relations stored in a list of Relations
    *
    * @return
    * <code>double</code> the sum of sizes of the potentials
    */
   public double sumSizes() {
      ...
      for (i = 0; i < relations.size(); i++) {
         ...
         //System.out.println("Relation considerada en sumSizes..........");
         //rel.print();
         //System.out.println("------------------------------------------");
    ...
    /**
    * Method to compute the total number of nodes
    * needed to represent all trees in a list of Relations
    *
    * @return
    * <code>double</code> the sum of sizes of the potentials
    */
   public double sumNumNodes() {
      Relation rel;
      Potential val;
      PotentialTree valTree;
      int i;
     ...
     /**
    * Gets the relations of a kind
    *
    * @param var a
    * <code>Node</code> variable.
    * @return a new
    * <code>RelationList</code> containing only the relations in this list that
    * are of the argument kind.
    */
   public RelationList getRelationsOfKind(int kind) {
    ...
    * contain the argument variable.
    */
   public RelationList getRelationsWithVarInTheHead(Node var) {
      ...
      for (i = 0; i < size(); i++) {
         rel = elementAt(i);
         if(rel.getValues().getHead() != null && rel.getKind() != Relation.CONSTRAINT){
            if (rel.getVariables().getId(var) != -1 && rel.getValues().getHead().indexOf(var) !=-1) {
                list.insertRelation(rel);
            }
         }
      }
   ...
   /**
    * Method to get the relations with non empty intersection given a set of
    * variables
    *
    ...
    * @return a new RelationList with the given set of relations
    */
    public RelationList getRelationsOf(Vector vars, int kind) {
        int i, j, p;
        Relation rel;
        RelationList list;
        Node var;
   ...
   /**
    * Method to get the relations whose first variable is in a given set of
    * variables
    *
    * @param vars Vector of variables to look for
    * @return a new RelationList with the given set of relations
   ...
   /**
    * Method to get the relations with non empty intersection given a
    * configuration. For this configuration we will consider its variables, and
    * look if there is non-empty intersection between the variables of the
    * configuration with a known value (not -1) and the variables of the
    * relation
    *
   ...
   /**
    * Obtain the relations that contain a given variable and remove them from
    * the list.
    *
    * @param var a
    * <code>Node</code> variable.
    ...
    *
    * @param var a
    * <code>Node</code> variable @flag to show the kind of relations to get:
    * utility or another. This method will not return constraint relations
    * @return a new
    * <code>RelationList</code> containing only the relations in this list that
    * contain the argument variable.
    */
   ...
   /**
    * Determines whether a variable is contained in only one relation in the
    * list.
    *
    * @param var a
    * <code>FiniteStates<code> variable to search for.
    * @return 1 if
    * <code>var</code> is in one relation, 0 if
    * <code>var</code> is not contained in any relation and -1 if
    * <code>var</code> is contained in more than one relation.
    */
    ...
    * @return
    * <code>true</code> if
    * <code>var</code> is in at least one of the relations.
    */
    ...
    * @return the position in the argument list of the node producing the
    * minimum size when being removed.
    */
    ...
    *
    * @param conf the
    * <code>Configuration</code> to restrict.
    * @return a
    * <code>RelationList</code> with the restricted relations.
    */
   ...
   /**
    * Restricts the relations in the list to the given configuration, except a
    * given goal variable.
    *
    * @param conf the
    * <code>Configuration</code> to restrict.
   ...
   /**
    * Restrict every relation to the set of evidences. Each observed variable
    * will be removed from the list of variables of the potential and from the
    * list of variables of the relation. The new potential will contain only the
    * values that are compatible with the evidence. If one observed variable is
    * the first in the list of variables of the relation, then the new relation
    * will be inactive
 * (
    * <code>active</code> = false). Otherwise,
    * <code>active</code> will remain unchanged.
    *
   ...
   /**
    * Removes constant relations. This method remove all the constant relations,
    * i.e. relations without variables and that contains a single value, by
    * multiplying the first non-constant relation by the product of all the
    * constant-relations values found.
    */
    ...
    * @return the relation in the list which name is
    * <code>nameOfRelation</code>. If such relation is not in the list, returns
    * <code>null</code>.
    */
    ...
    *
    * @param nameOfNode the name of child node of the relation that we are
    * looking for.
    * @return the relation in the list which name is
    * <code>nameOfRelation</code>. If such relation is not in the list, returns
    * <code>null</code>.
   ...
   /**
    * Updates the set of
    * <code>PotentialFunction</code>s in the list by assigning each argument in
    * the potential given by a string to the related Potential.
    */
   ...
   /**
    * Enter evidence in potentials by making 0 those values which disagree with
    * observations
    */
   ...
   public void changeKind(int oldKind, int newKind) {
       
       for(int i=0; i<relations.size(); i++) {
           if(((Relation)relations.elementAt(i)).getKind()==oldKind)
               ((Relation)relations.elementAt(i)).setKind(newKind);
       }
   }
   
   
   public void setHeadsAndTails(int kind) {
       

       
       for(int i=0; i<relations.size(); i++) {
           if(((Relation)relations.elementAt(i)).getKind()==kind){
             ((Relation)relations.elementAt(i)).setHeadAndTail();
           }
   ...
   public void removeUtilityVariable() {
          for(int i=0; i<relations.size(); i++) {
            if(((Relation)relations.elementAt(i)).getKind()==Relation.UTILITY){
               NodeList vars = ((Relation)relations.elementAt(i)).getVariables();
               vars.removeNodes(0);
               ((Relation)relations.elementAt(i)).setVariables(vars);
               ((Relation)relations.elementAt(i)).getValues().setVariables(vars.toVector());
