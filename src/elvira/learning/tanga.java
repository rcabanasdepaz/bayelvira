...
public class tanga extends ClassifierLearning {
...
//DataBaseCases cases;
 ...
 public static void main(String args[]) throws ParseException, IOException { 
     ...
     int i,j,k;
      ...
      baprend = (Bnet) cases;
      tanga outputNet1 = new tanga(cases,baprend);
	...
	//There are no cycles in this individual, which represents the best BN structure
      ...
      for(i=0 ; i< baprend.getNodeList().size()-1; i++){
        j= baprend.getNodeList().size()-1;
		System.out.println("Va a crear el link...: ");
              outputNet1.nodei = (Node) baprend.getNodeList().elementAt(i);
              outputNet1.nodej = (Node) baprend.getNodeList().elementAt(j);
            try{
     	        baprend.createLink(outputNet1.nodej,outputNet1.nodei);
            }catch (InvalidEditException e){};
      }

      for(i=0 ; i< baprend.getNodeList().size()-1; i++){
        for(j=0 ; j< baprend.getNodeList().size()-1; j++){
         if (i==j) continue;
		...
		//¿Comprobación de ciclos?
              outputNet1.nodei = (Node) baprend.getNodeList().elementAt(i);
              outputNet1.nodej = (Node) baprend.getNodeList().elementAt(j);
              if (!baprend.hasCycle(outputNet1.nodei,outputNet1.nodej)) {
               try{
    ...
    public tanga(DataBaseCases cases,Bnet b1){
...
//------------------------------------------------------------------------
   ...
   public Bnet learn(int tamPop, int maxIter, int crossProb) {
      ...
      for(i=0 ; i< baprend.getNodeList().size()-1; i++){
         ...
         try{
     	      baprend.createLink(nodej,nodei);
         }catch (InvalidEditException e){};
      }
      ...
      for(i=0 ; i< baprend.getNodeList().size()-1; i++){
         for(j=0 ; j< baprend.getNodeList().size()-1; j++){
            ...
            if (BestIndBits[k]==1) {// i parent of j
               ...
               nodei = (Node) baprend.getNodeList().elementAt(i);
               nodej = (Node) baprend.getNodeList().elementAt(j);
               if (!baprend.hasCycle(nodei,nodej)) {
                  try{
...
void InicAleatListaBin  (int[] sol)
{
	...
	for(i=0;i<Math.sqrt(LONGITUD);i++)
	{
		sol[i] = 0;
	}	
	for(i=1;i<Math.sqrt(LONGITUD);i++)
	{
		sol[i] = 1;
	}	
	for(i=(int) Math.sqrt(LONGITUD);i<LONGITUD;i++)
	{
		sol[i] = BitRand();
	}	
}
			    ...
			    int i2, int h1, int h2, double pc)
{
	...
	if(pc > doubleRand())
	{
		punto = (int) Math.sqrt(LONGITUD)+intRand(LONGITUD- (int) Math.sqrt(LONGITUD));
		for(i=0;i<punto;i++)
		{
			(Pobl[h1])[i] = (Pobl[i1])[i];
			(Pobl[h2])[i] = (Pobl[i2])[i];
...
void MutaListaBin (int[] indiv, double pm)
{
	...
	if(pm < doubleRand())
	{
		gen = (int)  Math.sqrt(LONGITUD)+intRand(LONGITUD-(int)Math.sqrt(LONGITUD));
...
{double IndValue=0.0;
 ...
 for(i=0 ; i< currentBnet.getNodeList().size()-1; i++){
   j=currentBnet.getNodeList().size()-1;
         nodei = (Node) currentBnet.getNodeList().elementAt(i); //baprend1 ???
         nodej = (Node) currentBnet.getNodeList().elementAt(j);
 	 newLink = new Link(nodej,nodei);
 ...
 for(i=0 ; i< currentBnet.getNodeList().size()-1; i++){
   for(j=0 ; j< currentBnet.getNodeList().size()-1; j++){
    if (j==i) continue;
