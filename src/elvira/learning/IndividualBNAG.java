...
/**
 * Implementa un individuo para algoritmos geneticos en el aprendizaje de
 * redes Bayesianas
 *
 * @version 0.1
 * @since 11/11/00
...
public IndividualBNAG(double fit,int N){
...
public IndividualBNAG(int N, double np, Random generador){
  ...
  for (i=0 ; i < orden.size() ; i++){
    ...
    // tocamos para limitar el numero de padres a N/5
    if (npar > Math.round(5.0)){ //(double)lChrom/5.0) ){
      npar = (int) Math.round(5.0); // (double)lChrom/5.0);
    }
