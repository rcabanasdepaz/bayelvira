...
public class StructuralMTELearning{
    ...
    public Vector setInitialBNet(Bnet net){
        ...
        //tengo que aï¿½adir una penalizacion por los cortes
        /*int numNodes;
        Node node;
        int numContVar = 0;
        double states = 1;
        ...
        //a la dimension le aï¿½ado lo que necesito para hacer los cortes
        dim = dim + penalSplit;
         */
    ...
    public Vector getBestNet(Bnet net, double oldQuality, double oldLikelihood, double oldPenal, double maxLastIterat) throws elvira.InvalidEditException{
        ...
        //System.out.println("Puedo aï¿½adir "+addLinks.size()+" arcos");
        ...
        //System.out.println("Busco el mejor movimiento entre los de aï¿½adir");
        ...
        for (int i = 0; i < addLinks.size(); i++){
            ...
            oldDim = (int) values[1];
            //try to add
            //System.out.println("Pruebo a aï¿½adir "+link);
    ...
    public Vector generateMove(Bnet net) throws elvira.InvalidEditException{
        ...
        //- aï¿½adir un enlace de los posibles
        ...
        ///System.out.println("Puedo aï¿½adir "+addLinks.size()+" arcos = "+numAddLinks(net));
        ...
        if (nMove < addLinks.size()){
            ...
            move.add(t);
            move.add(h);
            //System.out.println("Elijo aï¿½adir "+t.getName()+"->"+h.getName());
        }
    ...
    public Vector performMove(Bnet net, int movement, Link link, Vector oldResults) throws elvira.InvalidEditException{
        ...
        switch(movement){
            ...
            case ADD:
                //si elegimos aï¿½adir un enlace, solo hay q modificar lo q respecta a la cabeza
                var = link.getHead();
                values = new double[3];
                //ï¿½?esto no puede dar el q no debe, si cambia el linklist????
                //net.getLinkList().getID(link.getTail().getName(), link.getHead().getName());
                values = new double[3];
                values = getValues(var, net);
                oldLog = values[0];
                oldDim = (int) values[1];
                //try to add
                //System.out.println("aï¿½adir "+link);
                
                //System.out.println("Trato de crear el enlace");
                
                //si se puede aï¿½adir, lo aï¿½ado, sino elijo otros nodos
                try{
                    ...
                    isDag  = false;
                    //System.out.println("Numero de links: "+net.getLinkList().size());
                    //System.out.println("Al aï¿½adir ya no es un DAG ");
                    //net.removeLinkOnly(new Link(vars.elementAt(i), vars.elementAt(j)));
                }
...
//        int nRepet = Integer.valueOf(args[4]).intValue();
     ...
     * if(args.length < 4){
     * System.out.println("too few arguments: Usage: RedOriginal Tamaï¿½oDBC iteracionEmpieza Results.txt");
     * System.exit(0);
     * }
     *
