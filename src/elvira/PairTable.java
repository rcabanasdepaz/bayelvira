 ...
 * (
 * <code>FiniteStates,Vector</code>), where the
 * <code>Vector</code> contains
 * <code>Relations</code> where the
 * <code>FiniteStates</code> variable appears. It can be used in propagation
 * algorithms to control the relations in which a variable takes part, for
 * instance, when the variable is going to be deleted.
 *
 * @since 22/9/2000
 */
public class PairTable {
     ...
     * @return the list of relationships containing this variables
     */
    public Vector getRelationsWithVarInTheHead(Node var) {

        Vector relVar = getRelations(var);
        Vector relHeadVar = new Vector();
        Relation r = null;
        
        if(relVar != null) {
            for(int i=0; i<relVar.size(); i++) {
                r = (Relation) relVar.elementAt(i);
                if(r.getHead().getId(var) != -1) {
                    relHeadVar.add(r);
                }
            }
        }
    ...
    /**
     * Inserts an element at the end of the list. The element is a
     * <code>Node</code> variable with no relations. It is assumed that the
    ...
    /**
     * Inserts a relation in the position corresponding to the argument variable
     * var, which is assumed to be in the list.
     *
    ...
    /**
     * Inserts a relation at all the positions corresponding to variables in the
     * domain of the relation.
     *
     * @param rel a
     * <code>Relation</code> to insert.
     */
    public void addRelation(Relation rel) {
        ...
        for (i = 0; i < info.size(); i++) {
            ...
            //    node.relations.add(0, rel);
        }
    }

    /**
     * Removes a relation from all the positions where it appears.
     *
    ...
    /**
     * Returns the next node to remove according to the criterium of minimum
     * size.
     *
    ...
    /**
     * Returns the next node to remove according to the criterium of minimum
     * size, and taking into account that the deletion sequece obtained is a
     * constrained deletion sequence, where the nodes of the argument list have
     * to be the nodes placed at the end of the sequence.
     *
     * @param set the list of nodes that must be removed only after the rest of
    ...
    /**
     * Returns the next node to remove according to the criterium of minimum
     * size. The node must be different of the argument.
     *
     * @param notRemovable a
     * <code>FiniteStates</code> variable that must not be deleted.
     * @return the variable to remove.
     */
    ...
    public FiniteStates nextToRemove(NodeList set, String criterion) {
        ...
        } else if (criterion.equals("minSize+minFillExpRest")) {
            candidates = getCandidates(set, "minSize");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillExpRest");
            }
        } else if (criterion.equals("minSize+minFillNotExp")) {
            candidates = getCandidates(set, "minSize");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillNotExp");
            }
        } else if (criterion.equals("CanoMoral+minFillExpRest")) {
            candidates = getCandidates(set, "CanoMoral");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillExpRest");
            }
        } else if (criterion.equals("CanoMoral+minFillNotExp")) {
            candidates = getCandidates(set, "CanoMoral");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillNotExp");
            }
        } else if (criterion.equals("minFill+minFillExpRest")) {
            candidates = getCandidates(set, "minFill");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillExpRest");
            }
        } else if (criterion.equals("minFill+minFillNotExp")) {
            candidates = getCandidates(set, "minFill");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillNotExp");
            }
        } else if (criterion.equals("minFillExpRest+minSize")) {
            candidates = getCandidates(set, "minFillExpRest");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minSize");
            }
        } else if (criterion.equals("minFillExpRest+CanoMoral")) {
            candidates = getCandidates(set, "minFillExpRest");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "CanoMoral");
            }
        } else if (criterion.equals("minFillExpRest+minFill")) {
            candidates = getCandidates(set, "minFillExpRest");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFill");
            }
        } else if (criterion.equals("minFillExpRest+minFillNotExp")) {
            candidates = getCandidates(set, "minFillExpRest");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillNotExp");
            }
        } else if (criterion.equals("minFillNotExp+minSize")) {
            candidates = getCandidates(set, "minFillNotExp");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minSize");
            }
        } else if (criterion.equals("minFillNotExp+CanoMoral")) {
            candidates = getCandidates(set, "minFillNotExp");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "CanoMoral");
            }
        } else if (criterion.equals("minFillNotExp+minFill")) {
            candidates = getCandidates(set, "minFillNotExp");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFill");
            }
        } else if (criterion.equals("minFillNotExp+minFillExpRest")) {
            candidates = getCandidates(set, "minFillNotExp");
            if (candidates.size() > 1) {
                candidates = getCandidatesOnlyFor(candidates, "minFillExpRest");
            }
     ...
     * criterion, and taking into account that the deletion sequece obtained is
     * a constrained deletion sequence, where the nodes in the argument list
     * have to be the nodes placed at the end of the sequence.
     *
     * @param set the list of nodes that must be removed only after the rest of
     * nodes have been removed.
    ...
    public NodeList getCandidates(NodeList set, String criterion) {
        ...
        for (i = 0; i < info.size(); i++) {
            ...
            } else if (criterion.equals("minFill")) {
                s = node.numberOfLinksToAdd();
            } else if (criterion.equals("minFillExpRest")) {
                s = node.numberOfLinksToAddBetweenExplanationAndRest(set);
            } else if (criterion.equals("minFillNotExp")) {
    ...
    /**
     * Returns a node list containing the nodes of set which mimimizes
     * criterion.
     *
     * @param set the list of nodes that must be removed only after the rest of
     * nodes have been removed
