 ...
 * @version 0.1 24/2/2009
 * Started by copying and pasting from DecisionTreeFrame
 */
public class PolicyTreeFrame extends JInternalFrame {
	 ...
	 */
	public PolicyTreeFrame(Rule rule, String title, ResourceBundle bundle) {
		setTitle(title);
		
		// DONE: sacar al mediator y recibirlo como parametro
		policyTreeViewer= new PolicyTreeViewer(rule, bundle);
		
		// Se lee el valor por defecto a expandir cuando se muestra un arbol
		int maxLevels= policyTreeViewer.getModel().getLevelProxy().getNumberOfLevels()-1;
		int seleccion= Math.min(ElviraGUIMediatorPT.expandedLevelsByDefault,maxLevels);
		...
		scrollPane.getViewport().add(policyTreeViewer);
	...
	class SymInternalFrame implements javax.swing.event.InternalFrameListener
	{
		...
		public void internalFrameClosed(javax.swing.event.InternalFrameEvent event) {
			...
			if (object == PolicyTreeFrame.this) {
				PolicyTreeFrame_internalFrameClosed(event);
			}
		}
	 ...
	 * @param event
	 */
	void PolicyTreeFrame_internalFrameClosed(javax.swing.event.InternalFrameEvent event)
	{
		Elvira.getElviraFrame().disablePolicyTreeToolbars();
	...
	public void setZoomFactor(double newZoomFactor) {
		if(newZoomFactor!=state.zoomFactor) {
			...
			policyTreeViewer.setZoomFactor(newZoomFactor);
		}
	}
