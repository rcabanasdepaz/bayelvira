...
//	protected String menuitemShowChancePrecisionText;
	 ...
	 * 
	 * @param rule Arbol de decision a mostrar
	 ...
	 */
	public PolicyTreeViewer(Rule rule, ResourceBundle bundle) {
		...
		cellRenderer= new PolicyTreeCellRenderer(); 
	 ...
	 */
	public PolicyTreeModel getModel() {
	...
	void initPopup(ResourceBundle bundle, PopupAdapter adaptee) {
		...
		menuitemExpandAll.setText(ElviraFrame.localize(bundle,"PT.ExpandAll.label"));
		menuitemCollapseAll.setText(ElviraFrame.localize(bundle,"PT.CollapseAll.label"));
		menuitemExpandOne.setText(ElviraFrame.localize(bundle,"PT.ExpandOne.label"));
		menuitemCollapseOne.setText(ElviraFrame.localize(bundle,"PT.CollapseOne.label"));
		menuCustomize.setText(ElviraFrame.localize(bundle,"PT.Customize.label"));
		...
		// Textos de los menus
	/*	menuitemShowUtilityPrecisionText=ElviraFrame.localize(bundle,"PT.UtilityPrecision.label");		
		menuitemShowUtilityPrecision.setText(menuitemShowUtilityPrecisionText);
		menuitemShowUtilityPrecision.setEnabled(false);
		menuitemIncUtilityPrecision.setText(ElviraFrame.localize(bundle,"PT.UtilityPrecision.Increase.label"));
		menuitemDecUtilityPrecision.setText(ElviraFrame.localize(bundle,"PT.UtilityPrecision.Decrease.label"));
		menuitemResetUtilityPrecision.setText(ElviraFrame.localize(bundle,"PT.UtilityPrecision.Reset.label"));
		menuitemResetAllUtilityPrecisions.setText(ElviraFrame.localize(bundle,"PT.UtilityPrecision.ResetAll.label"));
		
		menuitemShowChancePrecisionText=ElviraFrame.localize(bundle,"PT.ChancePrecision.label");
		menuitemShowChancePrecision.setText(menuitemShowChancePrecisionText);
		menuitemShowChancePrecision.setEnabled(false);
		menuitemIncChancePrecision.setText(ElviraFrame.localize(bundle,"PT.ChancePrecision.Increase.label"));
		menuitemDecChancePrecision.setText(ElviraFrame.localize(bundle,"PT.ChancePrecision.Decrease.label"));
		menuitemResetChancePrecision.setText(ElviraFrame.localize(bundle,"PT.ChancePrecision.Reset.label"));
		menuitemResetAllChancePrecisions.setText(ElviraFrame.localize(bundle,"PT.ChancePrecision.ResetAll.label"));
*/
		menuitemTitle.setText(ElviraFrame.localize(bundle,"PT.Description.UseTitle.label"));
		menuitemTitle.setState(true);
		menuitemResetDefaultDescription.setText(ElviraFrame.localize(bundle,"PT.Description.ResetAll.label"));
...
/*	    menuitemIncUtilityPrecision.addActionListener(adaptee);
	 ...
	 */
	protected void setPopupCustomItems(MouseEvent e, ValueBox r) {
	...
	/*	menuitemShowUtilityPrecision.setText(menuitemShowUtilityPrecisionText+prec);
		 ...
		 */
		//if( source.getParent() instanceof RuleChanceNode ) {
...
/*	*//** Configura el popup para que muestre las opciones disponibles cuando se
	 ...
	 */
	protected void setPopupCustomItems(MouseEvent e, RuleNode r) {
		// Se eliminan las opciones anteriores (pudieran ser de otro tipo de nodo)
		menuCustomize.removeAll();
	...
	*/
	...
	protected void utilityActionPerformed(String cmd) {
		...
		if( cmd.equals("utility.increase") ) {
			...
			//
			if( r instanceof SummaryBoxPT ) {
				AbstractNode n= ((SummaryBoxPT) r).getSource();
	...
	protected void descriptionActionPerformed(String cmd) {
		...
		if( cmd.equals("description.change")) {
			...
			// de dicho nodo, no contra la del cuadro
			//
			if( r instanceof ValueBox ) {
				r= ((ValueBox) r).getSource();
			}
			
			boolean useTitle= menuitemTitle.getState();
			cellRenderer.getDescriptionProxy().setUseTitle( (RuleNode) r, useTitle );
			getModel().fireNodesChanged(r);
		}
	 ...
	 */
	public void setDecisionTree(Rule policyTree) {
		this.policyTree= policyTree;

		/* Componente personalizado para acceder a la información en el Arbol de decision
		 * Juega el rol de 'model' en el patrón MVC que sigue esta clase
		 */
		setModel(new PolicyTreeModel(policyTree));
	...
	/**
	 * Getter sobre el atributo <code>policyTree</code>
	 *  
	 ...
	 * 		directamente la barra de scroll: se soluciono en el PolicyTreeFrame
	 */
	...
	class TreeExpansionAdapter implements TreeExpansionListener {
		...
		public void treeExpanded(TreeExpansionEvent event) {
			...
			if( tn1 instanceof ValueBox ) {
				RuleNode n= ((ValueBox) tn1).getSource();
		...
		private void showPopup(MouseEvent e) {
			...
			if( path != null ) {
				...
				if( node instanceof ValueBox ) {
					setPopupCustomItems(e, (ValueBox) node);
				}
				else {					
					setPopupCustomItems(e, (RuleNode) node);
				}
			}
			else {
