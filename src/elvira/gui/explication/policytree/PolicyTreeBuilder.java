...
public class PolicyTreeBuilder {
	...
	/*public static void main(String args[]) throws ParseException, IOException {
		...
		Rule policyTree;
		try {
			policyTree = PolicyTreeBuilder.getInstance().buildPT(diag,false);
		...
		PolicyTreeViewer tree= new PolicyTreeViewer(policyTree,bundle);
	 ...
	 */
/*	public AbstractCompositeNodePT buildPT(IDWithSVNodes diag) throws DTBuildingException {
		...
		return (AbstractCompositeNodePT) tree;
	}*/
	 ...
	 * TODO: por ahora es destructivo... ¿hacer copia antes de operar?
	 */
	public Rule buildPT(IDWithSVNodes diagram, FiniteStates decision) throws PTBuildingException {
			...
			boolean found;
			Potential auxPot;
			boolean pruneNullConfigurationsPT=true;
			
			
			Propagation propag = diagram.getPropagation();
			//if (!evaluated){
			if (!diagram.getIsCompiled()||(propag!=null)&&(propag.getClass()!=ArcReversalSV.class)){
				diagram.compile(3,null);
			}
			
			
			
			if (diagram.getPn()==null){
				diagram.obtainAndSetPN();
			}
			...
			found = false;
			auxRule = null;
			for (int i=0;(i<potOfPolicies.size())&&!found;i++){
				auxPot = potOfPolicies.get(i);
				
				PotentialTable auxPotTable = (PotentialTable) auxPot;
				Vector variables = auxPotTable.getVariables();
				FiniteStates auxDec = (FiniteStates) (variables.elementAt(variables.size()-1));
				
				if (auxDec.getName().equals(decision.getName())){
					found = true;
					auxDetPot = new DeterministicPotentialTable(auxPotTable);
					auxRule = new Rule(auxDetPot,diagram,auxElimOrder,pruneNullConfigurationsPT,diagram.getPn());
				}
	...
	/*protected AbstractNode getSuperValueStructure( ArcReversalSV_ID2DT eval, Node node, Configuration configuration) throws DTBuildingException {
		...
		if(node.getKindOfNode()!=Node.SUPER_VALUE) {
			throw PTBuildingException("Se esperaba un nodo de tipo supervalor");
		}
...
/*	protected AbstractNode buildTree( ArcReversalSV_ID2DT eval, Configuration configuration, int depth  ) throws DTBuildingException {
		...
		AbstractCompositeNodePT arbol= null;
		...
		if( node.getKindOfNode() == Node.CHANCE ) {
			...
			ChanceNodePT chanceNode= new ChanceNodePT(node,configuration.duplicate());
			...
			arbol= new DecisionNodePT(node, configuration.duplicate());
			int numStates= node.getNumStates();
