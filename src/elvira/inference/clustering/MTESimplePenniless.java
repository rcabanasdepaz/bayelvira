...
public static void main(String args[]) throws ParseException, IOException {
  ...
  if (args.length < 9) {
    ...
    for(i=0 ; i < b.getRelationList().size() ; i++){
      ...
      //System.out.println("el nï¿½mero de tï¿½rminos es  "+contPot.getNumTerms()+" y el de splits es "+contPot.getNumSplits());
      ...
      contRel.setKind(0);
      System.out.println("El tamaï¿½o real del potencial "+i+" es: "+contPot.actualSize());
    ...
    //System.out.println("Results tiene tamaï¿½o: "+propagation.results.size());
...
public MTESimplePenniless(Bnet b, Evidence e, double delta, double epsilon, double epsilonJoin, double epsilonDisc, int prune2Int){
    ...
    //System.out.println("Este es el tamaï¿½o del irTree: "+irTree.size());
...
public void initMessages(double delta, double epsilon, double epsilonJoin, double epsilonDisc) {
  ...
  for (i=0 ; i<binTree.getJoinTreeNodes().size() ; i++) {
   ...
   // System.out.println("Tamaï¿½o real del potencial asociado al clique "+i+" del JoinTree: "+((PotentialContinuousPT)r.getValues()).actualSize());
...
public void sendMessage(NodeJoinTree sender, NodeJoinTree recipient, double delta, double epsilon, double epsilonJoin, double epsilonDisc, int prune2Int) {
  ...
  pot = aux;
  System.out.println("El que hay en el clique tiene tamaï¿½o: "+pot.actualSize()+" y es:");
  //pot.print();
  if((willMarginaliseToOne(sender,recipient))){
	  ...
	  if (label != recipient.getLabel()) { 
	      ...
	      //inside = true;
	      System.out.println("Multiplico por uno de tamaï¿½o: "+((PotentialContinuousPT)rel.getOtherValues()).actualSize()+" que es ï¿½ste:");
		  ...
		  if(howMany == 1){
		   if(pot.actualSize() == 1){//El potencial del nodo es 1
			 ...
			 System.out.println("Ahora podamos, tras la combinaciï¿½n");
      ...
      //System.out.println("Al final de combinar queda con tamaï¿½o, antes de poda (y marginalizaciï¿½n): "+pot.actualSize());
      //if ((delta != 0 ) || (epsilon != 0) || (epsilonDisc != 0) || (epsilonJoin != 0)){
...
//	}
  ...
  //System.out.println("Tras marginalizacion (y poda en su caso) queda con tamaï¿½o : "+pot.actualSize());
...
public void initRelations(JoinTree binTree, Bnet b,Evidence e, double delta, double epsilon, double epsilonJoin,double epsilonDisc, int prune2Int){
  ...
  while(rCopy2.size() > 0){
    ...
    //rel.print();
    //System.out.println("Quiero insertar un potencial de tamaï¿½o "+((PotentialContinuousPT)rel.getValues()).actualSize());
    rCopy2.removeRelationAt(biggerPos);
    
    //Este nuevo relationList tiene las relaciones ordenadas por tamaï¿½o
  ...
  for(h=0 ; h < rCopy3.size() ; h++){
	...
	if ((delta != 0 ) || (epsilon != 0) || (epsilonDisc != 0) || (epsilonJoin != 0)){
      ...
      nlNod = new NodeList();
      nlNod = node.getVariables();
      //Ahora aï¿½ado el potencial newPot al nodo
	...
	for(i=1 ; i<newVec.size() ; i++){// ahora para todos los posibles cliques vemos cual es el mejor
	  nj = binTree.elementAt(((Integer)newVec.elementAt(i)).intValue());
	    if((!elegido)&& ((PotentialContinuousPT)nj.getNodeRelation().getValues()).isUnity()){
	      elegido = true;
	      posChosen = i; // Posicion dentro del vecPosiblesIndex
	      //System.out.println("3ï¿½ opcion");
	    }// End of if
	}// End of for
      }// End of if
      
      // ultima opcion
      if(!elegido){// Esta ultima opcion he de ver en que clique insertarlo seria mas favorable (menor tamaï¿½o)
	nj = binTree.elementAt(((Integer)newVec.elementAt(1)).intValue());
	relClique = nj.getNodeRelation();
	potClique =  (PotentialContinuousPT)relClique.getValues();
	size2 = potClique.actualSize();
	posChosen = 1;
	//System.out.println("Tamaï¿½o del primero: "+size2);
	for(i=2 ; i<newVec.size() ; i++){
	  nj = binTree.elementAt(((Integer)newVec.elementAt(i)).intValue());
	  relClique = nj.getNodeRelation();
	  potClique =  (PotentialContinuousPT)relClique.getValues();
	  newSize2 = potClique.actualSize();
	  //System.out.println("Tamaï¿½o del "+i+" : "+newSize2);
	  if(newSize2 < size2){// Hay uno de menor tamaï¿½o, nos quedamos con ese
	    posChosen = i;
	    size2 = newSize2;
	    //System.out.println("El "+i+" es de menor tamaï¿½o");
	  }
	}
	//System.out.println("4ï¿½ opcion");
      }
      //System.out.println("Lo he insertado en el clique "+((Integer)newVec.elementAt(posChosen)).intValue());
      // Este es el clique del JoinTree elegido
      node = binTree.elementAt(((Integer)newVec.elementAt(posChosen)).intValue());
	...
	one = false;
       ...
       for (k=0 ; k<sender.getNeighbourList().size() ; k++) {
		...
		if (label != recipient.getLabel()) { 
			...
			if(!itIs){ 
			  System.out.println("Se quita la primera, luego serï¿½ uno la marginal");
