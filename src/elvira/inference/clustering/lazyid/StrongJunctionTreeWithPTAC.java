...
public class StrongJunctionTreeWithPTAC extends StrongJunctionTreeWithPT {
   ...
   public StrongJunctionTreeWithPTAC(IDiagram diag, double threshold, int triangulationCriteria, 
                                     ...
                                     boolean generateDebugInfo, boolean generateStatistics, LazyPropagationIDWithPTAC propagationID) {
      super(diag, threshold, triangulationCriteria, propagationCriteria, variableEliminationCriteria, generateDebugInfo, generateStatistics, propagationID);
   }
   ...
   public JunctionTreeNode buildTreeNode(StrongJunctionTree tree, NodeList clique, int index) {
      if (generateDebugInfo) {
         System.out.println("StrongJunctionTreeWithPACT: buildTreeNode ----- BEGIN");
      }
      JunctionTreeNode node=new JunctionTreeNodeWithPTAC(tree, clique, index, generateDebugInfo);
      if (generateDebugInfo) {
         System.out.println("StrongJunctionTreeWithPTAC: buildTreeNode ----- END");
      }
      return node;
   }
   ...
   public void addConstraintLinks(Graph graph) {
      ...
      if (generateDebugInfo) {
         System.out.println("StrongJunctionTreeWithPACT: addConstraintLinks ----- BEGIN");
      }
      ...
      if (generateDebugInfo) {
         System.out.println("StrongJunctionTreeWithPACT: addConstraintLinks ----- END");
      }
   }
