...
public class SamplingOnId extends Propagation{
   ...
   * strategy, but not as a result of a classification, but analyzing the
  ...
  public SamplingOnId(IDiagram diag, String phases, String samples){
     ...
     // Create the vector with the optimal strategy, without classification
  ...
  private void reconsiderLastDecision(){
    ...
    // strategy for it was evaluated with criteria
  ...
  private boolean addContext(Vector limitedCases){
    ...
    for(i=0; i < limitedCases.size(); i++){
      ...
      pastTree=(PotentialTree) ((IDiagram)network).applyConstraintsOnPotential(pastTree,false); 
   ...
   * optimal policies in the classifiers with optimal strategy
  ...
  private Configuration generateRandomSample(Vector order){
     ...
     // be stored the optimal strategy for the configuration defined
  ...
  private void evaluateNewSamples(CaseListMem cases){
     ...
     for(i=firstSample; i < lastSample; i++){
        ...
        //cases.setValue((int)i,0,strategy.getValue(decision));
...
System.out.println("A evaluar: "+(lastSample-firstSample));
    for(i=firstSample; i < lastSample; i++){
      ...
      // Set the strategy in conf to check if this configuration
      // is already stored
  ...
  private void buildClassifiers(){
     ...
     // a) one for the optimal strategy with optimality
     // b) one for the optimal strategy, taking into account all the
     //    branches
     // c) one for maximization (with C4.5)
     // d) one for sampling (with Dirichlet)
     
     for(i=0; i < decisions.size(); i++){
        // Create the classifier for optimal strategy with optimality
...
System.out.println("Maximo: "+maximum+" Minimo: "+minimum+" Umbral: "+thresholdOfProximity);
  ...
  public void classifyOptimalPolicyForFirstDecision(){
    ...
    // Store the tree with the exact optimal strategy, without classification
  ...
  private void classifyOptimalPolicyForDecisionWithPast(int index){
     ...
     // Build the potential tree with the optimal strategy withou classification
     ...
     // Store the tree with the exact strategy
  ...
  private boolean checkIfOptimalInPast(int index, Configuration conf){
     ...
     for(i=0; i < index; i++){
        ...
        // The configuration is related to an optimal strategy if the value
        // is not 0
   ...
   *        several values related to the optimal strategy
  ...
  */
  ...
  /**
   * Method to print the optimal strategy for the decisions of the influence
   * diagram
   */
   ...
   * @param <code>Configuration</code> configuration related to the selected strategy
   ...
   *        strategy (may be several alterntives with the same
  ...
  public Vector compareExpectedUtilities(Vector resultsToCompare,
    ...
    for(i=0; i < decisions.size(); i++){
      ...
      // Get the tree describing the optimal strategy for this decision
      ...
      for(k=0; k < cases; k++){
        ...
        // Get the vector with the optimal strategy related to this configuration.
	...
	// optimal branches
        ...
        // Begin with branches related to optimal strategy
        
        if (checkAllCeros(optimalPolicies) != true){
          // This is a configuration related to the optimal strategy
          ...
          // It is a branch not related to the optimal strategy, but anyway, look
   ...
   * @param <code>Vector</code> Vector with the approx strategy for the conf
  ...
  private double getGlobalUtility(boolean repeated, int indMax, Vector repetitions,
    ...
    // Get the prob related to optimal strategy
    ...
    // If there are not repetitions, return the utility for the optimal strategy
