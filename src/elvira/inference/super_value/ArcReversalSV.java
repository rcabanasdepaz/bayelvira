...
public class ArcReversalSV extends ArcReversal {
    ...
    /**
     * Tables of decisions that let to obtain the politics for each decision The
     * attribute 'results' will have the table of utilities for the parents of
     * the decision
     */
    ...
    /**
     * Check whether the diagram is suitable to be evaluated: - only directed
     * links - only one terminal value node - no cycles - directed path between
     * decisions and value node
     *
     * @return
    ...
    /**
     * To know if all the children of a node are utilities. It must have some
     * children.
     */
    ...
    /**
     * To know whether the decision can be removed in a value node. The
     * conditional predecessors of the value node must be informational
     * predecessors of the decision Precondition: All the sucessors of the
     * decision are utilitie nodes
     *
     * @param the decision node to be removed.
     * @param a value node.
     * @return the result of the operation.
    ...
    /**
     * To know whether the decision can be removed in the parents of the value
     * node that are reachable from the decision. The conditional predecessors
     * of the value node must be informational predecessors of the decision
    ...
    /**
     * Evaluates the Influence Diagram associated with the class using Tatman
     * and Shachter's Algorithm.
     *
     * @throws IOException
     */
     ...
     *
     * int type; int sizeParents; boolean isAppliedRule=false; Node auxI; Node
     * auxJ; int i; NodeList auxNodeList=new NodeList(); Node auxNode;
     *
     * type = value.getKindOfNode();
     *
     * //Subset rule is only applied to sv nodes if (type == Node.SUPER_VALUE){
     * NodeList parents = diag.parents(value); sizeParents=parents.size(); if
     * (sizeParents>0){ //Subset rule is applied to sv nodes whose parents are
     * utility //nodes if (parents.elementAt(0).getKindOfNode()==Node.UTILITY){
     * //Check if there are two parents of value to apply the //subset rule for
     * (i=0;(i<sizeParents-1)&&(isAppliedRule==false);i++){
     * auxI=parents.elementAt(i); for (int
     * j=i+1;(j<sizeParents)&&(isAppliedRule==false);j++){
     * auxJ=parents.elementAt(j); if (verifySubsetRule(auxI,auxJ)){ String
     * operation = "Apply subset rule to: " + auxI.getName()+ " and "+
     * auxJ.getName();
     *
     * statistics.addOperation(operation); System.out.println(operation);
     *
     * auxNodeList.insertNode(auxI); auxNodeList.insertNode(auxJ);
     * auxNode=introduceSVNode(auxNodeList,value);
     * ReductionAndEvalID.reduceNode((IDWithSVNodes)diag,auxNode);
     *
     * statistics.addSize(diag.calculateSizeOfPotentials());
     ...
     * }
     * }
     * else{ //Invoke applySubsetRule until it's applied to some //ancestor for
     * (i=0;(i<sizeParents)&&(isAppliedRule==false);i++){
     * isAppliedRule=applySubsetRule(parents.elementAt(i)); } } }
     * else{//Error:The sv node hasn't any parents System.out.println("Error in
     * function applySubsetRule in class ArcReversalSV\n");
     * System.out.println("SV node without parents\n\n"); isAppliedRule=false; }
     * } /*else{ System.out.println("Error in function applySubsetRule in class
     * ArcReversalSV\n"); System.out.println("Subset rule must be invoked on sv
     * nodes\n\n"); isAppliedRule=false; 
		}
     */
    /*
     ...
     *
     * @return the result of the operation, as a
     * <code>boolean</code>.
     * @throws IOException
     */
    @Override
...
//					 To store the size
    ...
    /**
     * It returns the value node which is descendent of 'node' in all the paths
     * from 'node', so it can be reduced by Tatman and Shachter's algorithm in
     * order to eliminate a chance or decision variable. No optimizations are
     ...
     * from 'node', so it can be reduced by Tatman and Shachter's algorithm in
     * order to eliminate a chance or decision variable. No optimizations are
     * applied in this method. See method ...
     */
    ...
    /**
     * To know whether a value node is a junction of all the paths from a chance
     * or decision node. Precondition: All the sucessors of the decision are
     * utilitie nodes
     *
     ...
     * @param inherit <ul> <li>1 (the parents of the node to remove will be
     * inherited). <li>0 (the parents of the node to remove will not be
     * inherited). </ul>
     */
     ...
     * of Shachter's algorithm can be applied to eliminate a chance or decision
     * node. This method creates (if it's necessary) a new node which is made
