...
public class ArcReversal extends elvira.inference.Propagation {
    ...
    /** Vector containing the policy for each decision. A policy is an
     object of the kind Hashtable<Configuration, Vector<Integer>> */
    ...
    public boolean initialConditions() {
        ...
        double numUtils = diag.getNodesOfKind(Node.UTILITY).size();
        
        evaluable = numUtils>0;
        
         //Checks if it contains multiple utilities and combine them
        if(numUtils>1) {
            ((IDiagram)network).combineMultipleUtilities();
             diag = ((IDiagram)network).copy();
            
        }
        
        if (evaluable == false) {
            System.out.print("Influence Diagram with 0 value nodes\n\n");
            return (false);
        }
    ...
    public void evaluateDiagram() {
        ...
        crono.start();
         ...
         //Checks if the ID is evaluable
        if(!initialConditions()) {
            System.out.println("ERROR: the ID is not evaluable");
            System.exit(0);
        }   
    ...
    public void getExpectedUtility(Node util, Node toRemove) {
        ...
        this.addGenPotential(expected, "Remove chance "+toRemove.getName()+": "+expected.toString());
    ...
    public void maximizeUtility(Node util, Node toRemove, RelationList tables) {
        ...
        this.addGenPotential(maximum, "Remove Decision "+toRemove.getName()+": "+maximum.toString());
    }
    ...
    public void getPosteriorDistributions(Node origin, Node dest) {
        ...
        this.addGenPotential(finalOrigin, "Arc Reversal final destination "+finalOrigin.toString());
        this.addGenPotential(finalDest, "Arc Reversal final origin "+finalDest.toString());
