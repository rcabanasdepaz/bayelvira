...
import elvira.tools.ProbComparator;
...
public class VEWithPotentialBPTree extends VariableElimination {   
   ...
   /**
     * Indicates if transformation for trees are only performed at the
     * begining or after each operation
     */
    protected boolean onlyInitialTransformation = false;
    
    /**
     * Data structure that stores initial trees from previous
     * querries. It is used to avoid re-building the trees
     */ 
    protected HashMap<String, Relation> relationCache;
   ...
   public Relation transformInitialRelation(Relation r) {
      ...
      //It checks first if the relation is in the cache
       if(relationCache.containsKey(r.getVariables().toString())){
   //        System.out.println("relation in cache");
           return relationCache.get(r.getVariables().toString()).copy();
       }
           
       
       
 //     System.out.println("Transform initial relations");       
      Relation rNew;
 ...
 //     System.out.println(potTree.getNumberOfNodes());
     ...
     double error = ProbComparator.getMeanKB(potTree, initTree);
     if(generateStatistics)
         statistics.addInitialProbError(error);
      
      
      rNew.setValues(potTree);
   //   System.out.println("-->"+potTree.getNumberOfNodes());

    //Updates the number of nodes for representing
    //all the initial potentials
    if(generateStatistics)
        statistics.setInitialPotentialsNodes(statistics.getInitialPotentialsNodes()+potTree.getNumberOfNodes());
          
      
      
      relationCache.put(rNew.getVariables().toString(), rNew.copy());
...
System.out.println(pot.getNumberOfNodes());
      if(sortVariablesInTrees){
...
//         pot.print();
  ...
  System.out.println("-->"+pot.getNumberOfNodes());
   ...
   /**
    * Initializes the relation cache
    */
    @Override
    public void initRelationCache() {
//       System.out.println("initRelationCache");
       relationCache = new HashMap<String, Relation>();
   }
 ...
 //      System.out.println("eraseRelationCache");
       relationCache.clear();
   }    
