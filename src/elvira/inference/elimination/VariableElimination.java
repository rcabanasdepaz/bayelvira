 ...
 * </ul>
 * @author Antonio Salmerï¿½n (Antonio.Salmeron@ual.es)
...
public class VariableElimination extends Propagation {
   ...
   protected String criteria = "minSize";
   ...
   public Potential addVariable(Potential pot, Node var) {
      ...
      if (pot.getHead() != null) {
                 newHead = Potential.getMargHead(pot, var);
                 newTail = Potential.getMargTail(pot, var);
      }
       
       
       
      newPot = pot.addVariable(var);
      newPot.setHead(newHead);
      newPot.setTail(newTail);
      return newPot;
   }
   
      /**
    * Removes the argument variable maximizing over all its values. This
    * method can be overloaded in subclases for special requirements.
    * @param pot a <code>Potential</code> over which the operation will
    * be carried out.
    * @param var a <code>Node</code> variable to be removed.
   ...
   public Potential maxVariable(Potential pot, Node node) {
      Vector newHead = null, newTail=null;
      Potential newPot;
      
      
      Vector vars = new Vector(pot.getVariables());
     vars.removeElement(node);
       
      if (pot.getHead() != null) {
                 newHead = Potential.getMargHead(pot, node);
                 newTail = Potential.getMargTail(pot, node);
      }
       

       
      newPot = pot.maxMarginalizePotential(node);
      newPot.setHead(newHead);
      newPot.setTail(newTail);
      return newPot;
   }
    ...
    //  System.out.println("Initial Relationships " + currentRelations.size());
      ...
      for (i=notRemoved.size() ; i>0 ; i--) {
         ...
         if (rLtemp.size() > 0) {
          ...
          //      System.out.println(getCurrentRelations().size() + " potentials");

               statistics.addNumberNodes(getCurrentRelations().sumNumNodes()+pot.getNumberOfNodes());
                statistics.addTotalSize(getCurrentRelations().sumSizes()+pot.getSize());
...
/**
   ...
   public void restrictCurrentRelationsToObservations() {
    ...
    public void generateStatistics(boolean b) {
